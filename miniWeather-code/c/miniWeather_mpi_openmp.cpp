...
double constexpr invdx = 1. / dx;
double constexpr invdz = 1. / dz;
///////////////////////////////////////////////////////////////////////////////////////
...
// so more feasible as ll + 4 * x + 4 * (nx + 1) * z
// #define FLUX_(ll, z, x) flux[NUM_VARS * (nx + 1) * (z) + NUM_VARS * (x) + (ll)]
...
// initial configurations
#define TEND_(ll, z, x) tend[(ll)*nz*nx + (z)*nx + (x)]
...
// inline __attribute__((always_inline))
double pow_pole(double base, double exponent, bool probe_max=false) {
  // return pow(base, exponent);
  static double _base_max = -NC_MAX_DOUBLE;
  if( probe_max) [[unlikely]] {
    return _base_max;
  }
  // return pow(base, exponent);
  // #pragma omp critical
  ...
  POW_X0_BRANCH(360)
  // POW_X0_BRANCH(420)
  // POW_X0_BRANCH(100) // 120, 180 mid = 100 * 1.5 = 150
  // // POW_X0_BRANCH(180)
  // // POW_X0_BRANCH(260)
  // // POW_X0_BRANCH(340)
  // // POW_X0_BRANCH(400)
  // POW_X0_BRANCH(140) // 180, 240 mid = 210 = 140 * 1.5
  // POW_X0_BRANCH(180) // 240, 300 mid = 270 = 180 * 1.5 
...
bool send_issued = false;
void send_halo_values_x( double *state) {
  int k, ll, ind_r, ind_u, ind_t, i, s, ierr;
  double z;
  if(!send_issued) [[unlikely]] {
    send_issued = true;
  } else {
    CHECK_MPI(MPI_Waitall(2,req_s,MPI_STATUSES_IGNORE));
  }
  ...
  for (ll=0; ll<NUM_VARS; ll++) {
    for (k=0; k<nz; k++) {
      for (s=0; s<hs; s++) {
        ...
        sendbuf_r[ll * nz * hs + k * hs + s] = STATE(ll, k + hs, nx + s); // nx + hs - (hs + s)
      }
    }
  }
  //Prepost receives
  CHECK_MPI(MPI_Irecv(recvbuf_l,hs*nz*NUM_VARS,MPI_DOUBLE, left_rank,0,MPI_COMM_WORLD,&req_r[0]));
  CHECK_MPI(MPI_Irecv(recvbuf_r,hs*nz*NUM_VARS,MPI_DOUBLE,right_rank,1,MPI_COMM_WORLD,&req_r[1]));
  //Fire off the sends
  CHECK_MPI(MPI_Isend(sendbuf_l,hs*nz*NUM_VARS,MPI_DOUBLE, left_rank,1,MPI_COMM_WORLD,&req_s[0]));
  CHECK_MPI(MPI_Isend(sendbuf_r,hs*nz*NUM_VARS,MPI_DOUBLE,right_rank,0,MPI_COMM_WORLD,&req_s[1]));
}

void recv_halo_values_x(double *state) {
  //Wait for receives to finish
  CHECK_MPI(MPI_Waitall(2,req_r,MPI_STATUSES_IGNORE));
...
#pragma omp parallel for collapse(3)
  ...
  if constexpr(data_spec_int == DATA_SPEC_INJECTION) {
    if (myrank == 0) {
...
int main(int argc, char **argv) {
  ...
  output(state,etime);
  IO_time += std::chrono::duration_cast<std::chrono::microseconds>(
    std::chrono::steady_clock::now() - io_t1
  ).count();
  ////////////////////////////////////////////////////
  // MAIN TIME STEP LOOP
  ////////////////////////////////////////////////////
  ...
  while (etime < sim_time) {
    ...
    auto intg_t0 = std::chrono::steady_clock::now();
    perform_timestep(state,state_tmp,flux,tend,dt);
    auto intg_t1 = std::chrono::steady_clock::now();
    intg_time += std::chrono::duration_cast<std::chrono::microseconds>(
      intg_t1 - intg_t0).count();
    //Inform the user
#ifndef NO_INFORM
    ...
    if (output_counter >= output_freq) {
      ...
      output(state,etime);
      IO_time += std::chrono::duration_cast<std::chrono::microseconds>(
        std::chrono::steady_clock::now() - io_t2
      ).count();
    }
  }
  ...
  if (mainproc) {
    ...
    std::cout << "Initialization time: " << init_time / 1e6 << " seconds." << std::endl; 
    std::cout << "Output FILE IO time: " << IO_time / 1e6 << " seconds." << std::endl; 
    std::cout << "Integration  time: " << intg_time / 1e6 << " seconds." << std::endl; 
  }
  ...
  // std::cout << "Trapped max base: " << pow_pole(0, true) << std::endl;
  finalize();
}
  ...
  for (ll=0; ll<NUM_VARS; ll++) {
    for (k=0; k<nz; k++) {
      for (i=0; i<nx; i++) {
        ...
        // tend[indt] = -(FLUX_(ll, k+1, i) - FLUX_(ll, k, i)) * invdz;
