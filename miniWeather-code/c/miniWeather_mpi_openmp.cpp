...
void flux_x_zx(double *state, double *flux, double hv_coef, double dt, int z, int x) {
  ...
  for (int ll=0; ll<NUM_VARS; ll++) {
    ...
    for (int s=0; s < sten_size; s++) {
      // int inds = ll*(nz+2*hs)*(nx+2*hs) + (z+hs)*(nx+2*hs) + x+s;
      stencil[s] = STATE(ll, z + hs, x + s); //state[inds];
    }
  ...
  r = vals[ID_DENS] + hy_dens_cell[z+hs] /*hy_dens_cell_khs*/;
  u = vals[ID_UMOM] / r;
  w = vals[ID_WMOM] / r;
  ...
  FLUX_(ID_UMOM, z, x) = r*u*u+p - hv_coef*d3_vals[ID_UMOM];
  FLUX_(ID_WMOM, z, x) = r*u*w   - hv_coef*d3_vals[ID_WMOM];
